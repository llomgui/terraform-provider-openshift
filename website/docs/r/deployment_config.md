---
layout: "openshift"
page_title: "Openshift: openshift_deployment_config"
subcategory: "Apps"
description: |-
...
* `trigger` - (Optional) Drive the creation of new deployment processes in response to events inside the cluster.

### `strategy`

#### Arguments

* `type` - Type of deployment. Can be 'Rolling' or 'Recreate' or 'Custom'. Default is Rolling.
* `rolling_update` - Rolling update config params. Present only if type = Rolling.
...
* `type` - Type of deployment. Can be 'ImageChange' or 'ConfigChange'. Default is ConfigChange.
* `image_change_params` - Rolling update config params. Present only if type = Rolling.

### `image_change_params`

#### Arguments

* `automatic` - (Optional) Automatic means that the detection of a new tag value should result in an image update inside the pod template.
* `container_names` - (Required) ContainerNames is used to restrict tag updates to the specified set of container names in a pod. If multiple triggers point to the same containers, the resulting behavior is undefined. Future API versions will make this a validation error. If ContainerNames does not point to a valid container, the trigger will be ignored. Future API versions will make this a validation error.
* `from` - (Optional) From is a reference to an image stream tag to watch for changes. From.Name is the only required subfield - if From.Namespace is blank, the namespace of the current deployment trigger will be used.

### `from`

#### Arguments

* `kind` - (Optional) Kind of the referent
* `name` - (Optional) Name of the referent
* `namespace` - (Optional) Namespace of the referent

## Timeouts

The following [Timeout](/docs/configuration/resources.html#operation-timeouts) configuration options are available for the `openshift_deployment_config` resource:
...
```
$ terraform import openshift_deployment_config.example default/terraform-example
```
